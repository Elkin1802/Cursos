...
?>
	...
	if (isset($_POST['guardar'])) {
		...
		if (!empty('identificacion') && !empty($nombres) && !empty($apellidos) && !empty($pais) && !empty($telefono) && !empty($email) && !empty($clave)) {
			if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
				...
				$consulta_insert = $conexion->prepare('INSERT INTO profesor(identificacion,nombres,apellidos,pais,telefono,email,clave) VALUES(:identificacion,:nombres,:apellidos,:pais,:telefono,:email,:clave)');
	...
	<body>
		<div class="contenedor">
			<h2>Insertar Un Nuevo Profesor</h2>
